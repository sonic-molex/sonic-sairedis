...
namespace syncd
{
    ...
    #define FLEX_COUNTER_EXT_IMPLEMENT()                            \
    ...
    {                                                               \
        ...
        {                                                           \
            return std::make_shared<CounterContext<sai_ocs_port_stat_t>>(context_name, instance, (sai_object_type_t)SAI_OBJECT_TYPE_OCS_PORT, vendorSai.get(), statsMode);\
        }                                                           \
                                                                    \
        if (context_name == FlexCounterExt::COUNTER_TYPE_OCS_CROSS_CONNECT_ATTRS)\
        {                                                           \
            return std::make_shared<AttrContext<sai_ocs_cross_connect_attr_t>>(context_name, instance, (sai_object_type_t)SAI_OBJECT_TYPE_OCS_CROSS_CONNECT, vendorSai.get(), statsMode);\
        }                                                           \
                                                                    \
        if (context_name == FlexCounterExt::COUNTER_TYPE_OCS_PORT_ATTRS) \
        {                                                           \
            return std::make_shared<AttrContext<sai_ocs_port_attr_t>>(context_name, instance, (sai_object_type_t)SAI_OBJECT_TYPE_OCS_PORT, vendorSai.get(), statsMode);\
        }                                                           \
                                                                    \
        return nullptr;                                             \
    }                                                               \
